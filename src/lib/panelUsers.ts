...
/** Returns the user's role if username+password match a panel_users row, else null. */
export async function authenticatePanelUser(
  username: string,
  password: string,
): Promise<{ role: PanelRole; username: string } | null> {
  try {
    ...
    if (!ok) return null;
    return { role: data.role as PanelRole, username: data.username as string };
  } catch {
    return null;
  }
}
